...
Two ways to use this from an MCP client, same `generate_pdf` tool either way:

- **Hosted (Streamable HTTP)** — add `https://paperpress.up.railway.app/mcp` directly to your client's MCP config (Claude Desktop/Claude.ai connectors, Cursor's `mcp.json`, Claude Code). No install. See `src/routes/mcp.ts`.
- **Stdio package** (`mcp/`) — not published to npm, so `npx` won't resolve it; build from source instead. See [mcp/README.md](mcp/README.md).

Both are thin — they call the same `/v1/documents` this REST API exposes, so credits and auth work identically either way.
